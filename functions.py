...
def choose_loot_type(rarity):
    ...
    empty_lanes = [lane for lane in range(constants.layout.num_lanes) if lane not in state.birds.random_lanes or all(state.birds.lost[idx] or state.birds.random_lanes[idx] != lane for idx in range(constants.layout.num_balls))]
    ...
    if random.random() < egg_prob:
        ...
        def allowed(egg_name):
            ...
            cnt = sum(1 for i in range(constants.layout.num_balls) if not state.birds.lost[i] and state.birds.colors[i] == bcol)
            return cnt < limit
...
def set_ball_vy(idx, val):
    ...
    immediate post-fire protection window (state.special.purple_just_fired_frames > 0), we
    ...
    try:
        if 0 <= int(idx) < constants.layout.num_balls:
            if state.special.purple_state[idx] == 2 or (state.special.purple_just_fired_frames and state.special.purple_just_fired_frames[idx] > 0):
                return
...
def get_scared_frames(bird_idx, base_seconds=2.0):
    """Return number of frames for state.special.scared_birds for bird_idx.
...
def transform_bird_to_s(bi):
    ...
    if label == 'S':
        ...
        if target_color is not None:
            ...
            if state.birds.colors[bi] == target_color and not state.birds.lost[bi]:
                # cnt already includes it; nothing else to do
                pass

            # If limit is None (unlimited) or cnt < limit we can transform
            if limit is None or cnt < limit or (state.birds.colors[bi] == target_color):
...
def perform_shuffle(count: int):
    ...
    for _ in range(max(0, int(count))):
        ...
        living_sorted = sorted(living_available, key=lambda i: abs(state.birds.random_lanes[i] - center), reverse=True)
        ...
        lost_slots = [i for i in range(constants.layout.num_balls) if state.birds.lost[i] and i not in used_lost_slots]
        if lost_slots:
            empty_lanes = sorted([state.birds.random_lanes[i] for i in lost_slots], key=lambda l: abs(l - center))
            target_lane = empty_lanes[0]
            # find the lost slot index that holds this lane (and not used yet)
            target_lost_idx = next((li for li in lost_slots if state.birds.random_lanes[li] == target_lane), None)
            if target_lost_idx is not None:
                # Swap lanes between the source bird and the lost slot
                state.birds.random_lanes[src_idx], state.birds.random_lanes[target_lost_idx] = state.birds.random_lanes[target_lost_idx], state.birds.random_lanes[src_idx]
                # Update rendering columns and reset moved bird to starting line facing up
                state.birds.cols[src_idx] = constants.layout.lane_positions[state.birds.random_lanes[src_idx]]
                state.birds.y[src_idx] = constants.layout.starting_line
                set_ball_vy(src_idx, -1)
                reset_bird_power(src_idx)
                state.birds.cols[target_lost_idx] = constants.layout.lane_positions[state.birds.random_lanes[target_lost_idx]]
        ...
        inner_candidates = [i for i in living if i != src_idx and i not in moved_indices and abs(state.birds.random_lanes[i] - center) < abs(src_lane - center)]
        if inner_candidates:
            # Choose the one closest to center
            tgt_idx = sorted(inner_candidates, key=lambda i: abs(state.birds.random_lanes[i] - center))[0]
            state.birds.random_lanes[src_idx], state.birds.random_lanes[tgt_idx] = state.birds.random_lanes[tgt_idx], state.birds.random_lanes[src_idx]
            # Update both moved birds' rendering cols and reset positions to starting line
            state.birds.cols[src_idx] = constants.layout.lane_positions[state.birds.random_lanes[src_idx]]
            state.birds.y[src_idx] = constants.layout.starting_line
            set_ball_vy(src_idx, -1)
            reset_bird_power(src_idx)
            state.birds.cols[tgt_idx] = constants.layout.lane_positions[state.birds.random_lanes[tgt_idx]]
            state.birds.y[tgt_idx] = constants.layout.starting_line
        ...
        state.birds.random_lanes[src_idx], state.birds.random_lanes[other] = state.birds.random_lanes[other], state.birds.random_lanes[src_idx]
        state.birds.cols[src_idx] = constants.layout.lane_positions[state.birds.random_lanes[src_idx]]
        state.birds.y[src_idx] = constants.layout.starting_line
        set_ball_vy(src_idx, -1)
        reset_bird_power(src_idx)
        state.birds.cols[other] = constants.layout.lane_positions[state.birds.random_lanes[other]]
        state.birds.y[other] = constants.layout.starting_line
        set_ball_vy(other, -1)
...
# ---------------- Level-from-state.game.score helpers ----------------
def compute_level_from_score(sc):
    # Start from state.game.level 1 and find highest state.game.level such that state.game.score >= threshold
    lvl = 1
    while True:
...
def add_score(amount, by_bird=None):
    """Add to global state.game.score. If by_bird is provided (bird index), also award XP to that bird.
    ...
    raw_amount = amount
    # Try to interpret as float for state.game.score math
    amt = float(amount)

    # Compute prestige multiplier (based on birds currently on field)
    prestige = compute_prestige()

    # Add multiplied state.game.score (only state.game.score is affected by prestige)
    ...
    achievements.check_achievements_event('score', score=state.game.score, frame_count=state.game.frame_count, notifications_list=state.ui.notifications, firebase_client=firebase_client, background_call=functions.background_call)

def award_xp(bird_idx, xp_amount):
    """Best-effort: credit XP to a bird without affecting global state.game.score.
...
def deduct_score(amount):
    ...
    if state.game.score < 0:
        state.game.score = 0
    # Re-evaluate state.game.score-based achievements when state.game.score changes
    achievements.check_achievements_event('score', score=state.game.score, frame_count=state.game.frame_count, notifications_list=state.ui.notifications, firebase_client=firebase_client, background_call=functions.background_call)
...
def handle_clockwork_auto_bounce():
    ...
    for i, c in enumerate(state.birds.colors):
        # Treat CLOCKWORK birds as special: only auto-bounce if they still have charge > 0
        if c == CLOCKWORK and not state.birds.lost[i] and state.birds.vy[i] == 1 and state.birds.y[i] >= constants.layout.starting_line:
            charge = state.special.clockwork_charge.get(i, None)
...
def find_bird_in_lane(lane):
    ...
    return state.birds.random_lanes.index(lane) if lane in state.birds.random_lanes else -1
...
def try_glitch_bounce(bird_idx):
    ...
    if state.birds.colors[bird_idx] == GLITCH:
...
def bounce_bird(bird_idx, apply_boost=False):
    ...
    if state.birds.colors[bird_idx] == CLOCKWORK:
        c = state.special.clockwork_charge.get(bird_idx, None)
        if c is None:
    ...
    if apply_boost and state.powerups.bounce_boost_active and bird_idx not in state.special.speed_boosts:
    ...
    achievements.append_recent_action('bounce', lane=state.birds.random_lanes[bird_idx], 
                                     color=state.birds.colors[bird_idx], frame_count=state.game.frame_count)

def can_bird_bounce(bird_idx):
    """Check if bird can be bounced (not scared, except PURPLE)."""
    if bird_idx in state.special.scared_birds and state.birds.colors[bird_idx] != PURPLE:
        return False
    return True
...
def update_orange_egg_lane(bird_idx, new_lane):
    ...
    if is_orange_egg_state(bird_idx):
        old_lane = state.birds.random_lanes[bird_idx]
...
def swap_bird_lanes(bird_idx1, bird_idx2):
    ...
    update_orange_egg_lane(bird_idx1, state.birds.random_lanes[bird_idx2])
    update_orange_egg_lane(bird_idx2, state.birds.random_lanes[bird_idx1])
    
    # Swap lanes
    state.birds.random_lanes[bird_idx1], state.birds.random_lanes[bird_idx2] = \
        state.birds.random_lanes[bird_idx2], state.birds.random_lanes[bird_idx1]
    
    # Update column positions
    state.birds.cols[bird_idx1] = constants.layout.lane_positions[state.birds.random_lanes[bird_idx1]]
    state.birds.cols[bird_idx2] = constants.layout.lane_positions[state.birds.random_lanes[bird_idx2]]
    
    # Reset birds to start if not lost and not in egg state
    for bird_idx in [bird_idx1, bird_idx2]:
        if not state.birds.lost[bird_idx] and not is_orange_egg_state(bird_idx):
...
def spawn_bird_from_egg(bird_color, loot_type):
    ...
    for idx in range(constants.layout.num_balls):
        if state.birds.lost[idx]:
            ...
            state.birds.speeds[idx] = int(bird_types.BALL_SPEEDS_DEFAULT.get(cname, 3))
