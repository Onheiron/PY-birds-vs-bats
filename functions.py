...
def choose_loot_type(rarity):
    ...
    empty_lanes = [lane for lane in range(variables.NUM_LANES) if lane not in state.random_lanes or all(state.ball_lost[idx] or state.random_lanes[idx] != lane for idx in range(variables.NUM_BALLS))]
    ...
    if num_empty == 0:
        egg_prob = 0.0
    elif num_empty in variables.EGG_PROBS:
        egg_prob = float(variables.EGG_PROBS[num_empty])
    else:
        # Fallback: choose the highest defined key
        int_keys = sorted([k for k in variables.EGG_PROBS.keys() if isinstance(k, int)])
        egg_prob = float(variables.EGG_PROBS[int_keys[-1]])
    ...
    if random.random() < egg_prob:
        ...
        def allowed(egg_name):
            bcol = variables.EGG_TO_COLOR.get(egg_name)
            if bcol is None:
                return True
            limit = variables.COLOR_LIMITS.get(bcol)
            if limit is None:
                return True
            # Count active birds of that color
            cnt = sum(1 for i in range(variables.NUM_BALLS) if not state.ball_lost[i] and state.ball_colors[i] == bcol)
            return cnt < limit

        # Candidate eggs and their base weights per rarity (weights are configurable)
        if rarity == 'common':
            rarity_data = variables.RARITY_EGGS_CANDIDATES.get('common', {})
            candidates = list(rarity_data.keys()) if isinstance(rarity_data, dict) else ['yellow_egg', 'red_egg', 'blue_egg']
            raw_weights = rarity_data if isinstance(rarity_data, dict) else variables.RARITY_WEIGHTS.get('common', {})
        elif rarity == 'uncommon':
            rarity_data = variables.RARITY_EGGS_CANDIDATES.get('uncommon', {})
            candidates = list(rarity_data.keys()) if isinstance(rarity_data, dict) else ['blue_egg', 'patchwork_egg', 'purple_egg']
            raw_weights = rarity_data if isinstance(rarity_data, dict) else variables.RARITY_WEIGHTS.get('uncommon', {})
        elif rarity == 'rare':
            rarity_data = variables.RARITY_EGGS_CANDIDATES.get('rare', {})
            candidates = list(rarity_data.keys()) if isinstance(rarity_data, dict) else ['white_egg', 'orange_egg', 'gold_egg']
            raw_weights = rarity_data if isinstance(rarity_data, dict) else variables.RARITY_WEIGHTS.get('rare', {})
        else:
            rarity_data = variables.RARITY_EGGS_CANDIDATES.get('epic', {})
            candidates = list(rarity_data.keys()) if isinstance(rarity_data, dict) else ['dinosaur_egg', 'glitch_egg']
            raw_weights = rarity_data if isinstance(rarity_data, dict) else variables.RARITY_WEIGHTS.get('epic', {})
...
def set_ball_vy(idx, val):
    ...
    """
    try:
        if 0 <= int(idx) < variables.NUM_BALLS:
...
def get_scared_frames(bird_idx, base_seconds=2.0):
    ...
    return max(1, int(float(base_seconds) / variables.base_sleep))
...
def transform_bird_to_s(bi):
    ...
    """
    bi = int(bi)
    if bi < 0 or bi >= variables.NUM_BALLS:
        return
    ...
    if label == 'S':
        ...
        if old == BLUE:
            target_color = WHITE
            target_speed = int(variables.BALL_SPEEDS_DEFAULT.get('WHITE', 4))
        elif old == RED:
            target_color = ORANGE
            target_speed = int(variables.BALL_SPEEDS_DEFAULT.get('ORANGE', 5))
        elif old == YELLOW:
            target_color = GOLD
            target_speed = int(variables.BALL_SPEEDS_DEFAULT.get('GOLD', 6))
        # If we have a mapping, check limits before applying
        if target_color is not None:
            limit = variables.TRANSFORM_LIMITS.get(target_color, None)
...
def perform_shuffle(count: int):
    ...
    for _ in range(max(0, int(count))):
        ...
        living = [i for i in range(variables.NUM_BALLS) if not state.ball_lost[i]]
        ...
        lost_slots = [i for i in range(variables.NUM_BALLS) if state.ball_lost[i] and i not in used_lost_slots]
        if lost_slots:
            ...
            if target_lost_idx is not None:
                ...
                state.ball_cols[src_idx] = variables.LANE_POSITIONS[state.random_lanes[src_idx]]
                state.ball_y[src_idx] = variables.STARTING_LINE
                set_ball_vy(src_idx, -1)
                reset_bird_power(src_idx)
                state.ball_cols[target_lost_idx] = variables.LANE_POSITIONS[state.random_lanes[target_lost_idx]]
        ...
        if inner_candidates:
            ...
            set_ball_vy(src_idx, -1)
            reset_bird_power(src_idx)
            state.ball_cols[tgt_idx] = variables.LANE_POSITIONS[state.random_lanes[tgt_idx]]
            state.ball_y[tgt_idx] = variables.STARTING_LINE
        ...
        set_ball_vy(src_idx, -1)
        reset_bird_power(src_idx)
        state.ball_cols[other] = variables.LANE_POSITIONS[state.random_lanes[other]]
        state.ball_y[other] = variables.STARTING_LINE
        set_ball_vy(other, -1)
...
def calculate_level_threshold(level):
    ...
    base = float(variables.LEVEL_SCORE_BASE)
    factor = float(variables.LEVEL_SCORE_FACTOR)
    return int(base ** (factor ** (level + 1)))
...
def compute_grade_from_xp(xp):
    ...
    # Use configurable progression constants
    base = float(variables.XP_BASE)
    factor = float(variables.GRADE_EXP_FACTOR)
...
def compute_prestige():
    ...
    for i in range(len(state.per_bird_xp)):
        ...
        add = variables.PRESTIGE_MODIFIERS.get(label, 0.0)
...
def adjust_rarity_weights(base_weights, prestige):
    ...
    """
    factor = 1.0 + float(prestige) * float(variables.PRESTIGE_RARITY_FACTOR)
